...
send_notification() {
    ...
    if [[ -n "${NOTIFICATION_EMAIL:-}" ]] && command_exists "mail"; then
        echo "$message" | mail -s "Backup Pyramid DB - $status" "$NOTIFICATION_EMAIL"
    fi

    if [[ -n "${WEBHOOK_URL:-}" ]] && command_exists "curl"; then
...
main() {
    ...
    # Define arquivo de backup APÓS carregar variáveis
    ...
    send_notification "SUCCESS" "Backup do banco $MYSQL_DATABASE realizado com sucesso em ${total_duration}s"
}
