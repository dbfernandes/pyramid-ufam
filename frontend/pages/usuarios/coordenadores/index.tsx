...
export default function Coordenadores() {
  ...
  useEffect(() => {
    if (page == 0) {
      router.replace(`/usuarios/coordenadores?page=1&search=${search}`);
      return;
    }
    if (page > 0) {
      fetchUsers();
  ...
  async function fetchUsers() {
    ...
    const options = {
      url: `${process.env.api}/users?type=coordenador&page=${page}&limit=15&search=${search}&courseId=${user.selectedCourse ? user.selectedCourse.id : ""}`,
      method: "GET",
      headers: {
