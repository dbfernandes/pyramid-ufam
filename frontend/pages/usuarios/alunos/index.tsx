...
export default function Alunos() {
  ...
  async function fetchUsers(_page, _search, _status, _sort, _order) {
    ...
    const options = {
      url: `${process.env.api}/users?type=aluno&page=${_page}&limit=${itensPerPage}&search=${_search}&courseId=${user.selectedCourse ? user.selectedCourse.id : ""}${parseUserActiveParam(_status)}${sort ? `&sort=${_sort}` : ""}${order ? `&order=${_order}` : ""}`,
      method: "GET",
      headers: {
