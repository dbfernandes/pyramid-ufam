...
export default function SolicitacoesGrupoAtividade() {
  ...
  useEffect(() => {
    if (page == 0) {
      router.replace(`/solicitacoes/grupo-atividade/${activityGroup}?page=1&search=${search}`);
      return;
    }
    if (page > 0) {
      fetchSubmissions();
  ...
  async function fetchSubmissions() {
    ...
    const options = {
      url: `${process.env.api}/courses/${user.selectedCourse?.id}/submissions?page=${page}&limit=15&search=${search}&activityGroup=${ActivityGroupsNames[activityGroup].slug}`,
      method: "GET",
      headers: {
